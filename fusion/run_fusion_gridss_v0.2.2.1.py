...
def GridssResult(sort_bam, gridss, gridss_jar, hg19_fa, blacklist, config):
    cmd_fusion = " ".join([
        gridss, '--threads 8',
        '--reference', hg19_fa,
        '--jar', gridss_jar,
...
def AddSVType(input_vcf):
    ...
    for line in open(input_vcf, 'r'):
        if line.startswith('#'):
            ...
            bk1_chrn = col[0]
            try:
                bk2_chrn = re.findall(r'(chr\w+):', col[4])[0]
            except IndexError:
                continue
            if '[' in col[4] and bk1_chrn == bk2_chrn:
                col[2] = 'DEL'
            elif ']' in col[4] and bk1_chrn == bk2_chrn:
                col[2] = 'INV'
            else:
                col[2] = 'CTX'
            optlist.append("\t".join(col))
    with open(f'{dirprefix}.fusion_svtype.vcf', 'w') as otopen:
...
def BedResult(anno_vcf):
    ...
    for line in os.popen(f"cat {anno_vcf} |grep -v '#'"):
        ...
        filters = col[6]
        svtype = col[2]
        pdlist.append( [ bk1, bk2, gene1, gene2, vf, sr, rp, refpair, ref, af, filters, svtype ] )
...
def Result(pdlist, hotdict):
    ...
    nonhotpassresult = f'{dirprefix}.sv_nonhottable.xls'
    column_list = ['bk1','bk2','gene1','gene2','VF','SR','RP','REFPAIR','REF','AF','FILTER','SVTYPE'] # type: str str str str int int int int int float str str
    hotdflist = []
    nonhotdflist = []
    for i,e in enumerate(pdlist):
        try:
        ...
        if case1 or case2:
            ...
            if e[-2] == 'PASS': #filters
                nonhotdflist.append(str(i+1)+'\t'+'\t'.join(e)+'\n')
    with open(nonhotpassresult, 'w') as otopen:
        otopen.write('\t'+"\t".join(column_list)+'\n')
        otopen.writelines(nonhotdflist)
    if hotdflist == []:
        with open(hotresult,'w') as otopen:
            otopen.write('\t'+"\t".join(column_list)+'\n')
        return hotresult, nonhotpassresult
    df = pd.DataFrame(hotdflist)
...
def ReportVcf(hotresult, nonhotpassresult, hotspot_table):
    allresult = os.popen("cat %s %s |awk '{if($4!=$5)print}' |grep -v SVTYPE"%(hotresult, nonhotpassresult)).readlines()
...
"""
    ...
    for line in allresult: # num bk1 bk2 gene1 gene2 VF SR RP REFPAIR REF AF FILTER SVTYPE
        if not line.startswith('\t'):
            ...
            #    af = col[-3]
            # else:
            #    af = str( float(alt_ad) / int(dp) )
            ref_ad = str( int(dp) - int(alt_ad) )
            # filter
            tag = 'PASS'
            if (int(svlen) < 200 and svlen != 0) or float(af) >= 1 or int(alt_ad) >= int(dp):
                continue
            else:
                if hottag is True and int(alt_ad) < 5:
                    tag = 'LQ'
                elif hottag is False and int(alt_ad) < 5:
                    continue
            # if int(alt_ad) < 5 or (int(svlen) < 200 and svlen != '0') or float(af) >= 1 or (int(alt_ad) >= int(dp)):
                # tag = 'LQ'
                # continue
                # if hottag is not True: # non hot
                #    continue
                # else: # hot
                #    dp = str( int(dp) + int(alt_ad) )
                #    alt_ad = col[5]
                #    af = col[10]
                #    ref_ad = str( int(dp) - int(alt_ad) )
                #    tag = 'LQ'
            # elif int(alt):
                # tag = 'PASS'
            optstr = f"{chr1}\t{pos1}\tN\t.\t<{alt}>\t100\t{tag}\tSOMATIC;BND=LR;SVTYPE={alt};CHR2={chr2};END={pos2};SVLEN={svlen}\tGT:DP:AD:AF\t0/1:{dp}:{ref_ad},{alt_ad}:{af}\n"
            optlist.append(optstr)
...
def Running(sort_bam, cfg, output_dir, output_prefix, hotspot):
    ...
    # 这里加入对*.fusion_raw.vcf文件的判断如果存在不为0则不执行
    fusion_raw_vcf = f"{dirprefix}.fusion_raw.vcf"
    fusion_svtype_vcf = f"{dirprefix}.fusion_svtype.vcf"
    anno_vcf = f'{dirprefix}.hg19_multianno.vcf'

    if bac.GetSize(fusion_raw_vcf) == 0:
        GridssResult(sort_bam, gridss, gridss_jar, hg19_fa, blacklist, config)
    os.system(f"chmod 755 {output_dir}")
    AddSVType(fusion_raw_vcf)
    # fusion_svtype_vcf = f"{dirprefix}.fusion_svtype.vcf"
    
    if bac.GetSize(anno_vcf) == 0:
        TableAnnovar(perl, table_annovar, annovar_humandb, fusion_svtype_vcf)
    ...
    (hotresult, nonhotpassresult) = Result(pdlist, hotdict)
    ReportVcf(hotresult, nonhotpassresult, hotspot)
