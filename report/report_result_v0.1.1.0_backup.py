...
def SnvIndelHot(multianno_vcf, hotdict):
            ...
            if keys != []:
                ...
                for k in uniq_keys:
                    if hotdict.get(k) is not None:
                        ...
                        optstr = "\t".join( [ hotdict.get(k), k.replace(':','').split(',')[1], k.replace(':','').split(',')[0], l[0], l[1], l[3], l[4], dp, ad, af, dp4 ] ) + '\n'
...
def Running(multianno_vcf, cfg, output_dir, output_prefix):
    ...
    aflist, snvindellist = SnvIndelHot(multianno_vcf, hotdict)
    ...
    with open(f'{dirprefix}.snvindel.hotspot.result.xls','w') as otopen:
        otopen.writelines(['CELL\tGENE\tHGVS.p\tCHROM\tPOS\tREF\tALT\tDP\tAD\tAF%\tDP4\n'] + sort_snvindellist)
