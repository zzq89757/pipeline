...
def calc_copynumbers(baseline, tumor, estimateTP, output):
    ...
    df_merge.to_csv(f"{output}/{tumor.columns.values[1]}_cnvbin.txt", sep="\t", index = True, index_label="bin_info")
    df_genes = pd.DataFrame(gene_copynumbers)
    df_genes.insert(df_genes.shape[1], "TP", f"{estimateTP*100}%")
    df_genes.to_csv(f"{output}/{tumor.columns.values[1]}_eachgene_tmp.txt", sep="\t", index = True, header=True)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"{args.targetcov}") and args.bam: # 经过测试后args.bam和args.targetcov都存在 或 仅有args.targetcov都跳过这一步
        print("-"*20,f"没有的targetcov，从bam文件中重新计算...")
