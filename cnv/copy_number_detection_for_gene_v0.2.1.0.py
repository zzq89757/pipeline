...
global config
config = bac.ResolveConfig(cfgfile)
...
def get_parser():
    ...
    parser.add_argument('-r', '--refbaseline', dest="baseline", type=str, required=True,
                        help='copy number baseline file, required=TRUE')
    parser.add_argument('-b', '--bam', dest="bam", type=str, default=False,
                        help='bam file of tumor sample, default=False')
    parser.add_argument('-t', '--targetcoverage', dest="targetcov", type=str, default=False, 
                        help='target coverage of tumor sample, if "bam" parameter is False "targetcoverage" file must exists, default=False')
    parser.add_argument('-d', '--bed', dest="bed", type=str, default=False, 
                        help='panel bed file of tumor sample, if "targetcoverage" parameter exists bed file not need, default=False')
    parser.add_argument('-o', '--output', dest="output", type=str, required=False, default=".",
                        help='file path of output results, default="."')
...
# 计算肿瘤样本中的coverage
def calc_tumorcov(targetbed, tumor_sample, output):
    # 计算tumor sample的Hsmetrics
    path = os.path.split(tumor_sample)[0]
    prefix = os.path.split(tumor_sample)[1].split(".")[0]

    cmd_picard_hs = " ".join([
        config['language']['java'], f"-jar -Xmx{config['language']['java_mem']}G",
        config['software']['picard'], 'BedToIntervalList',
        '-I', targetbed,
        '-O', f"{output}/{prefix}.BedToIntervalList.xls",
        '-SD', config['database']['hg19_dict'],
        '\n',
        config['language']['java'], f"-jar -Xmx{config['language']['java_mem']}G",
        config['software']['picard'], 'CollectHsMetrics',
        '-I', tumor_sample,
        '-O', f"{output}/hsmetrics_{prefix}.txt",
        '-R', config['database']['hg19_fa'],
        '-PER_TARGET_COVERAGE', f'{output}/{prefix}.targetcov',
        '-BAIT_INTERVALS', f'{output}/{prefix}.BedToIntervalList.xls',
        '-TARGET_INTERVALS', f'{output}/{prefix}.BedToIntervalList.xls'
    ])
    bac.RunningProcess(cmd_picard_hs, output, prefix, 'Notice:\tcnv detect used picard CollectHsMetrics running messages')
    # os.system(f"{config['language']['java']} -jar -Xmx{config['language']['java_mem']}G {picard} \
    # BedToIntervalList \
    #     -I {targetbed} \
    #     -O {output}/{prefix}.BedToIntervalList.xls \
    #     -SD {config['database']['hg19_dict']}")
    
    # os.system(f"{config['language']['java']} -jar -Xmx{config['language']['java_mem']}G {picard} \
    # CollectHsMetrics \
    #     -I {tumor_sample} \
    #     -O {output}/hsmetrics_{prefix}.txt \
    #     -R {config['database']['hg19_fa']} \
    #     -PER_TARGET_COVERAGE {output}/{prefix}.targetcov \
    #     -BAIT_INTERVALS {output}/{prefix}.BedToIntervalList.xls \
    #     -TARGET_INTERVALS {output}/{prefix}.BedToIntervalList.xls")
...
def renorm(df_coverage, df_normalized_coverage, binsizes, samplename):
    ...
    for sample in df_normalized_coverage.columns:
        ...
        # print (binsizes[condition.index].sum())
...
def calc_copynumbers(baseline, tumor, estimateTP, output):
    ...
    df_merge.to_csv(
        f"{output}/{tumor.columns.values[0]}_cnvgene.txt", sep="\t", index = True
    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"{args.targetcov}") and args.bam: # 经过测试后args.bam和args.targetcov都存在 或 仅有args.targetcov都跳过这一步
        print("-"*20,f"没有targetcov，从bam文件中重新计算...") #{prefix}的
        prefix = calc_tumorcov(args.bed, args.bam, args.output)
        args.targetcov = f"{args.output}/{prefix}.targetcov"
    else:
