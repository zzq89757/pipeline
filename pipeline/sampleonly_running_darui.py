...
def GetProcess(umi, running_step):
    allsteps = "trim\tfastqQC\tmapping\trmdup\tumi\treadligner\tbamQC\tsnvindel\tannotation\tfusion\tcnv\tmsi\treport\t"
    if umi == 'false':
...
def BuildCommands(fastq_read1, fastq_read2, bed, umi, umi_read1, umi_read2, UMI_type, cfg, output_dir, output_prefix):
    ...
    # snv_indel
    (tmp1, tmp2) = subprocess.Popen(f"rm {output_dir}/snv_indel/{output_prefix}_indelqual.ba*", shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE).communicate()
    ...
    running_name = ['TRIM', 'FASTQQC', 'MAPPING', 'RMDUP', 'UMI', 'BAMQC', 'READLIGNER', 'SNVINDEL', 'ANNOTATION', 'FUSION', 'CNV', 'MSI', 'REPORT']
    running_process = [cmd_trim, cmd_fastq_qc, cmd_mapping, cmd_rmdup, cmd_umi, cmd_bam_qc, cmd_indelrealigner, cmd_snv_indel, cmd_annotation, cmd_fusion, cmd_cnv, cmd_msi]
    need_files = (trimfq1, trimfq2, sort_bam, bamqc_checkfile, targetcoverage, indelrealigner_bam, pass_vcf, multianno_vcf)
...
# make directory
    dirlist = ['log', 'trim', 'QC', 'mapping', 'snv_indel', 'annotation', 'fusion', 'cnv', 'msi', 'report']
    for each in dirlist:
...
# process command
    ...
    (trimfq1, trimfq2, sort_bam, bamqc_checkfile, targetcoverage, indelrealigner_bam, pass_vcf, multianno_vcf) = need_files
# running process
    steps = GetProcess(umi, running_step)
    for x in range(len(running_name)):
        if running_name[x] not in steps:
            running_process[x] = 'skip'
    (cmd_trim, cmd_fastq_qc, cmd_mapping, cmd_rmdup, cmd_umi, cmd_bam_qc, cmd_indelrealigner, cmd_snv_indel, cmd_annotation, cmd_fusion, cmd_cnv, cmd_msi) = running_process
...
# for report
    ...
    os.system(f"cp {output_dir}/fusion/csv_{output_prefix}.sv.vcf {output_dir}/report/{output_prefix}")
    os.system(f"cp {output_dir}/cnv/csv_{output_prefix}.gene.xls {output_dir}/report/{output_prefix}")
    os.system(f"cp {output_dir}/msi/{output_prefix}.msi_result.tsv {output_dir}/report/{output_prefix}")
    getcsv = os.path.dirname(os.path.abspath(__file__)) + '/../caller/get_csv_for_single_darui.py'
    os.system(f"{python} {getcsv} {pass_vcf} {output_dir}/report/{output_prefix} {output_prefix}")
    # 非配对样本用get_csv_for_single直接生成*.merge_snv.[snv|indel].vcf文件，如果有配对样本则再重新生成一次文件，把这个文件覆盖掉
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--bed', type=str, default="%s/../bed/NanOnco_Plus_Panel_v2.0_Covered_hg19.bed"%localdir,
                        help='panel design bed file, default=%s/../bed/NanOnco_Plus_Panel_v2.0_Covered_hg19.bed'%localdir)
