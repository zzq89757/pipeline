...
def GetProcess(umi, running_step):
    allsteps = "trim\tfastqQC\tmapping\trmdup\tumi\treadligner\tbamQC\tsnvindel\tannotation\tfusion\tcnv\tmsi\t"
    if umi == 'false':
...
def BuildCommands(fastq_read1, fastq_read2, bed, umi, umi_read1, umi_read2, UMI_type, cfg, output_dir, output_prefix):
    ...
    # msi
    cmd_msi = " ".join([ python, command['msi'], 'detect', '--output_dir', f'{output_dir}/msi', '--output_prefix', output_prefix, config['parameter']['msi_basline'], indelrealigner_bam, '>', f'{output_dir}/msi/{output_prefix}.msi_result.tsv' ])
    # cnv
    ...
    if cnv_baseline.get(bed.split('/')[-1]) is not None:
        cmd_cnv = " ".join([ python, command['cnv'], '--refbaseline', cnv_baseline.get(bed.split('/')[-1]), '--bam', indelrealigner_bam, '--targetcoverage', targetcoverage, '--bed', bed, '--output', output_dir+'/cnv', '--vcf', multianno_vcf ])
    else:
    ...
    running_name = ['TRIM', 'FASTQQC', 'MAPPING', 'RMDUP', 'UMI', 'BAMQC', 'READLIGNER', 'SNVINDEL', 'ANNOTATION', 'FUSION', 'CNV', 'MSI']
