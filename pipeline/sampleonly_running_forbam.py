...
def GetProcess(umi, running_step):
    allsteps = "rmdup\tumi\treadligner\tbamQC\tsortbamsnvindel\tsnvindel\tannotation\tfusion\tcnv\tmsi\treport\t"
    if umi == 'false':
...
def BuildCommands(bam, bed, umi, umi_read1, umi_read2, UMI_type, cfg, output_dir, output_prefix):
    ...
    running_name = ['RMDUP', 'UMI', 'BAMQC', 'READLIGNER', 'SORTBAMSNVINDEL', 'SNVINDEL', 'ANNOTATION', 'FUSION', 'CNV', 'MSI', 'REPORT']
    running_process = [cmd_rmdup, cmd_umi, cmd_bam_qc, cmd_indelrealigner, cmd_snv_indel_sortbam,  cmd_snv_indel, cmd_annotation, cmd_fusion, cmd_cnv, cmd_msi, cmd_report]
    need_files = (sort_bam, bamqc_checkfile, targetcoverage, indelrealigner_bam, sortbam_multianno_vcf, pass_vcf, multianno_vcf)
    return running_name, running_process, need_files

def Running(bam, bed, umi, umi_read1, umi_read2, UMI_type, cfg, output_dir, output_prefix, running_step):
# make directory
...
# process command
    (running_name, running_process, need_files) = BuildCommands(bam, bed, umi, umi_read1, umi_read2, UMI_type, cfg, output_dir, output_prefix)
    (sort_bam, bamqc_checkfile, targetcoverage, indelrealigner_bam, sortbam_multianno_vcf, pass_vcf, multianno_vcf) = need_files
# running process
    steps = GetProcess(umi, running_step)
    for x in range(len(running_name)):
        if running_name[x] not in steps:
            running_process[x] = 'skip'
    (cmd_rmdup, cmd_umi, cmd_bam_qc, cmd_indelrealigner, cmd_snv_indel_sortbam, cmd_snv_indel, cmd_annotation, cmd_fusion, cmd_cnv, cmd_msi, cmd_report) = running_process
# process running
...
if __name__ == '__main__':
    ...
    parser.add_argument('-bam', '--bam', required=True,
                        help='after downsampled bam file, required')
    ...
    Running(args.bam, args.bed, args.umi, args.umi_read1, args.umi_read2, args.UMI_type, args.cfg, args.output_dir, args.output_prefix, args.running_step)
